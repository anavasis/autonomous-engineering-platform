...
/**
 * Shared external CLI execution pipeline for real agent binaries.
 *
 * Vendor providers (Cursor, and later Claude/Codex/Gemini) supply identity
 * and binary defaults; process lifecycle, streams, timeout, cancel, prompt
 * delivery, and ProviderResult mapping live here.
 *
 * Does not synthesize workspace changes — DiffCollector continues to read
 * RESULT.diff or baseline diffs exactly as today.
 */
class ExternalCliProvider extends AbstractBufferedProvider
{
    ...
    {
        $id = is_string($options['id'] ?? null) ? (string) $options['id'] : 'external-cli';
        $name = is_string($options['displayName'] ?? null) ? (string) $options['displayName'] : $id;
        $this->label = is_string($options['label'] ?? null) && trim((string) $options['label']) !== ''
            ? trim((string) $options['label'])
            : $name;

        $configured = is_string($options['binary'] ?? null) ? trim((string) $options['binary']) : '';
        if ($configured === '') {
            $envKey = is_string($options['envBinaryKey'] ?? null) ? (string) $options['envBinaryKey'] : '';
            if ($envKey !== '') {
                $env = getenv($envKey);
        ...
        if ($configured === '') {
            $defaultBinary = is_string($options['defaultBinary'] ?? null) ? trim((string) $options['defaultBinary']) : '';
            $configured = $defaultBinary !== '' ? $defaultBinary : 'external-cli';
        }
    ...
    public function health(): ProviderHealth
    {
        ...
        if ($resolved === null) {
            return new ProviderHealth(
                'unavailable',
                $this->label . ' binary not found or not executable: ' . $this->binary,
                Utc::now()
            );
        }

        return new ProviderHealth('ok', $this->label . ' ready (' . $resolved . ')', Utc::now());
    }
    ...
    protected function run(ProviderSessionRequest $request): void
    {
        ...
        if ($binary === null) {
            $this->complete(
                $sessionId,
                new ProviderResult(ProviderResult::REJECTED, $this->label . ' binary unavailable.')
            );
        ...
        $this->push($sessionId, 'log', $this->label . ' starting', [
            'binary' => $binary,
            'cwd' => $cwd,
        ...
        if (!is_resource($process)) {
            $this->complete(
                $sessionId,
                new ProviderResult(ProviderResult::FAILED, 'Unable to start ' . $this->label . ' process.')
            );
        ...
        while (true) {
            ...
            if (microtime(true) >= $deadline) {
                $timedOut = true;
                proc_terminate($process);
                $this->push($sessionId, 'log', $this->label . ' timed out; terminating process');
                break;
            }
        ...
        if ($timedOut) {
            $this->complete(
                ...
                new ProviderResult(
                    ...
                    $this->label . ' timed out after ' . $request->timeoutSeconds() . 's',
                    $filesChanged,
                    $diffText,
                    $usage,
                    $meta
        ...
        if ($exitCode !== 0) {
            $message = trim($stderr) !== '' ? trim($stderr) : ($this->label . ' exited with code ' . $exitCode);
            $this->complete(
                $sessionId,
                new ProviderResult(
        ...
        $this->complete(
            ...
            new ProviderResult(
                ...
                $this->label . ' completed ' . $request->action(),
                $filesChanged,
                $diffText,
                $usage,
                $meta
